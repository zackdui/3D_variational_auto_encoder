...
if __name__ == "__main__":
    ...
    if check_feasibility:
        ...
        # attn_params2 = AttnParams(num_heads=4, dim_head=32, dropout=0, window_size=None)
        # blocks_down=(1,)
        # blocks_up=()


        # model = CustomVAE(upsample_mode="deconv", debug_mode=True, vae_latent_channels=8, init_filters=8, beta=0, vae_use_log_var=True, blocks_down=blocks_down, blocks_up=blocks_up, use_attn=use_attn, attn_params=attn_params1)
        ...
        # norm=""
        model = CustomVAE(debug_mode=False, init_filters=64, act=act, upsample_mode="pixelshuffle", vae_latent_channels=4, beta=0, vae_use_log_var=True, blocks_down=blocks_down, blocks_up=blocks_up, use_attn=use_attn)
